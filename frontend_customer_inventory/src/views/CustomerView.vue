      ...
      <el-table :data="list" border stripe>
        ...
        <el-table-column prop="level" label="等级" width="110">
          <template #default="{ row }">
            <el-tag :type="row.level === '大客户' ? 'danger' : row.level === 'VIP' ? 'warning' : 'info'">
              {{ row.level || '普通' }}
            </el-tag>
          </template>
        </el-table-column>
...
const form = reactive<Customer>(emptyCustomer())
...
const openDialog = (row?: Customer) => {
  ...
  Object.assign(form, row || emptyCustomer())
...
const remove = async (id?: number) => {
  ...
  await ElMessageBox.confirm('确认删除该客户？有关联预约或订单时，后端会阻止删除。', '提示', { type: 'warning' })
